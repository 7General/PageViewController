...
#pragma mark - 懒加载
- (MSPageScrollView *)pageScrollView {
    if (!_pageScrollView) {
...
- (MSHeaderView *)headerView {
    if (!_headerView) {
        _headerView = [[MSHeaderView alloc] initWithFrame:CGRectMake(0, 0, kScreenW, kWYHeaderHeight)];
    }
    return _headerView;
}
...
- (NSArray *)childVCs {
    if (!_childVCs) {
        MSListViewController *hotVC = [MSListViewController new];
        
        MSListViewController *albumVC = [MSListViewController new];
        
        MSListViewController *videoVC = [MSListViewController new];
        
        MSListViewController *introVC = [MSListViewController new];
