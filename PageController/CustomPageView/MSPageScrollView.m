...
@interface MSPageScrollView() <UITableViewDataSource,UITableViewDelegate>
...
@implementation MSPageScrollView
...
- (void)initSubViews {
    ...
    self.mainTableView = [[MSPageTableView alloc] initWithFrame:CGRectZero style:UITableViewStylePlain];
...
- (void)configListViewScroll {
    [[self.delegate listViewsInPageScrollView:self] enumerateObjectsUsingBlock:^(id<MSPageListViewDelegate>  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)mainScrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (self.isCriticalPoint) {
        ...
        //如果允许列表刷新，并且mainTableview的offsetY小于0 或者 当前列表offsetY小于0
